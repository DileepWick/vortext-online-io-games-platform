...
const IncomeExpenseAnalysis = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const rentalPaymentsResponse = await axios.get(`${API_BASE_URL}/rentalPayments/`);
      
      setAllPaymentsData(allPaymentsResponse.data.orderHistory || []);
      setRentalPaymentsData(rentalPaymentsResponse.data.rentalPayments || []);
      
      calculateAnalysis(allPaymentsResponse.data.orderHistory, rentalPaymentsResponse.data.rentalPayments);
    } catch (error) {
  ...
  const calculateAnalysis = (allPayments, rentalPayments) => {
    ...
    allPayments.forEach(payment => {
      ...
      const discount = (payment.stockid?.discount / 100) * payment.stockid?.UnitPrice || 0;
      
      income += salePrice - devFunds;
      expenses += devFunds + discount;

      updateMonthlyStats(payment.date, salePrice - devFunds, devFunds + discount, monthlyStats);
    });
    ...
    rentalPayments.forEach(payment => {
      ...
      updateMonthlyStats(payment.date, amount - devFunds, devFunds, monthlyStats);
    });
  ...
  const generatePDF = async () => {
    ...
    // Add title
    pdf.setFontSize(18);
    pdf.text('Income and Expense Analysis Report', pdfWidth / 2, 20, { align: 'center' });
    ...
    pdf.setFontSize(12);
    pdf.text(`Total Income: Rs. ${analysisData.income.toFixed(2)}`, margin, 200);
    pdf.text(`Total Expenses: Rs. ${analysisData.expenses.toFixed(2)}`, margin, 210);
    pdf.text(`Net Profit: Rs. ${analysisData.netProfit.toFixed(2)}`, margin, 220);

    // Add percentage changes
    if (monthlyData.length > 1) {
      pdf.text('Percentage Changes (vs Previous Month)', margin, 235);
      pdf.text(`Income: ${calculatePercentageChange(monthlyData[monthlyData.length - 1].income, monthlyData[monthlyData.length - 2].income)}`, margin, 245);
      pdf.text(`Expenses: ${calculatePercentageChange(monthlyData[monthlyData.length - 1].expenses, monthlyData[monthlyData.length - 2].expenses)}`, margin, 255);
      pdf.text(`Net Profit: ${calculatePercentageChange(monthlyData[monthlyData.length - 1].netProfit, monthlyData[monthlyData.length - 2].netProfit)}`, margin, 265);
    }

    // Add note
    pdf.setFontSize(10);
    pdf.text('Note: Income is calculated as the sale price minus DevFunds for both regular and rental payments.', margin, pdfHeight - 20, { maxWidth: pdfWidth - 2 * margin });
    pdf.text('Expenses include discounts from regular sales and DevFunds from rental payments.', margin, pdfHeight - 15, { maxWidth: pdfWidth - 2 * margin });
      ...
      <CardFooter>
        <div className="w-full">
          ...
          {monthlyData.length > 1 && (
            <>
              <h4 className="text-lg font-semibold mt-4 mb-2">Percentage Changes (vs Previous Month)</h4>
              <p>Income: {calculatePercentageChange(monthlyData[monthlyData.length - 1].income, monthlyData[monthlyData.length - 2].income)}</p>
              <p>Expenses: {calculatePercentageChange(monthlyData[monthlyData.length - 1].expenses, monthlyData[monthlyData.length - 2].expenses)}</p>
              <p>Net Profit: {calculatePercentageChange(monthlyData[monthlyData.length - 1].netProfit, monthlyData[monthlyData.length - 2].netProfit)}</p>
            </>
          )}
