...
const MinimalistShop = () => {
  ...
  useEffect(() => {
    const fetchGameStocks = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/gameStocks/allGameStock`);
        if (!response.ok) {
    ...
    >
      ...
      <div className="relative max-w-4xl mx-auto text-center text-white z-10">
        ...
        <p className="text-xl text-white/90 mb-12 max-w-2xl mx-auto font-light">
          Curated collection of premium games with exclusive deals and instant
          downloads
        </p>
        <button
          onClick={() =>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <p className="text-gray-600 text-lg">
              ...
              {filteredStocks.length === 1 ? "game" : "games"} available
            </p>
          </div>
